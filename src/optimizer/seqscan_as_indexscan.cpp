      ...
      while (!seq_children.empty()) {
        ...
        for (const auto &seq_child : seq_children) {  // 遍历seq_children
          ...
          if (compare) {  // 若是compare，则判断是否能被优化
            ...
            // 记录查找到的常量
            auto rvalue = std::dynamic_pointer_cast<ConstantValueExpression>(rchild);

            if (lcolumn != nullptr && rcolumn != nullptr) {  // 若比较的左右child都为column，则不能被优化
              can_be_optimized = false;
              break;
            }

            if (lcolumn == nullptr &&
                rcolumn != nullptr) {  // 若右为column 左为值，则交换二者，即保证左一定为column 右一定为值
              lcolumn = std::move(rcolumn);
              rvalue = std::dynamic_pointer_cast<ConstantValueExpression>(lchild);
            }

            // 检验是否只有一个列
            uint32_t col = lcolumn->GetColIdx();
          ...
          } else {  // 若expression不是comparison，继续递归其children
        ...
        // 遍历新children
        seq_children = std::move(tem_children);
      }
